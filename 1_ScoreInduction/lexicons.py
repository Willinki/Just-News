#from socialsent.socialsent import constants
import util
import collections
import numpy as np
import itertools
...
def make_concreteness_lexicon(top=75, bottom=25):
    ...
    pos_thresh = np.percentile(list(raw_scores.values()), top)
    neg_thresh = np.percentile(list(raw_scores.values()), bottom)
    polarities = {}
...
def compare_lexicons(print_disagreements=False):
    ...
    for l1, l2 in itertools.combinations(list(lexicons.keys()), 2):
        ...
        common_words = set([word for word in list(ps1.keys()) if ps1[word] != 0]) & \
                       set([word for word in list(ps2.keys()) if ps2[word] != 0])  
